...
public class XYZ extends PApplet {
	...
	public void draw(){
		...
		if (robotMode && robot.isSetup() && !initialized){
			robot.setSpeed(550000, 500000, 500000, 500000);
			robot.setZone(Robot.z0);
			initialized = true;
			robot.getSpeed();
			robot.getRobTarget();
			//robot.setPosition(1000,-500,500);
		...
		if (mousePressed && robotMode){
		 ...
		 if (val != null) {
			 Comma = val.indexOf(',');
			 Xoffset = val.substring(0,Comma); 
			 Yoffset = val.substring(Comma+1,val.length());
			 Xval = Integer.parseInt(Xoffset.trim());
			 Yval = Integer.parseInt(Yoffset.trim());
			 println(Yval);
			 
			float[] xyz = robot.getPosition();
		    robot.setPosition((int)xyz[0]+(518-Xval)/5, (int)xyz[1]+(518-Yval)/5, (int)xyz[2]);
